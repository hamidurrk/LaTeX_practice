%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Activities}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

  \vspace{-0.3mm}
%---------------------------------------------------------
  \cventryalt
    {\textbf{Role in Organization}} % Affiliation/role
    {National Coach} % Location
    {World Robot Olympiad (WRO)} % Organization/group
    {Jan 2023 - Dec 2023} % Date(s)
    {
      \begin{cvitems} % Description(s) of experience/contributions/knowledge
        \item {Led a team in the Future Innovators segment (national), focusing on robot construction, software design, \& business analysis.}
        \item {Guided two national teams' participation in international competition rounds, one team achieved an impressive, first time in the history of Bangladeshi participants, a silver award out of 451 global teams at WRO ‘23 Panama.}
      \end{cvitems}
    }

    \vspace{-1.0mm}
%---------------------------------------------------------
\cventryalt
    {Supplemental Instructor of Computer Science} % Job title
    {Web Developer} % Location
    {Give Bangladesh Foundation} % Organization
    {Sept 2023 - Dec 2025} % Date(s)
    {
      \begin{cvitems}
      \item {Collaborated with African NGO, Awake Youth Initiative, \& built them a fundraising website for slum communities in Nairobi, Kenya.}
      \item {\underline{Leveraged knowledge} in Full Stack Web Development, Javascript, Git, Figma for UI/UX Design, SEO, and Google Analytics.}
      \end{cvitems}
    }
    \vspace{-1.0mm}
%---------------------------------------------------------
  \cventryalt
    {Role in Organization} % Affiliation/role
    {President, Robotics} % Location
    { Notre Dame IT Club} % Organization/group
    {Aug 2019 - June 2021} % Date(s)
    {
      \begin{cvitems} % Description(s) of experience/contributions/knowledge
        \item {Organized a 3-day national tech event with 1100+ participants, 20+ robotics competitions by leading a team of 70+ volunteers.}
        \item {Mentored 45+ national robotics/project teams, conducted 9 workshops along with 27+ robotics-oriented classes for 150+ students.}
        \item {Was commissioned by my school’s ICT dept to design and build a national curriculum-oriented teaching module: 
        a combined demonstration of clock pulse generators, logic gates, flip flops, registers, counters, and a 7-segment display on a breadboard.}
      \end{cvitems}
    }
%---------------------------------------------------------
\end{cventries}
